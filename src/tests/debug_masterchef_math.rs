...
#[wasm_bindgen_test]
fn test_simple_reward_calculation_debug() -> Result<()> {
    clear();
    
    // Use the exact same setup as the working boundary test but with simpler math
    ...
    // Mint tokens (exactly like boundary test)
    ...
    // Initialize vault with simple parameters (like boundary test)
                        ...
                        vec![
                            Protostone {
                                message: into_cellpack(vec![
                                    ...
                                    1000u128,      // reward_per_block (simple round number)
                                    3u128,         // start_block
                                    120000u128,    // preloaded_rewards (match what we have)
                                ...
                                edicts: vec![
                                    ProtostoneEdict {
                                        ...
                                        amount: 120000u128,  // Match preloaded_rewards exactly
                                        output: 1,
                                    }
                                ],
                            }
    ...
    // Create user token (separate mint)
    ...
    index_block(&mint_user, 10)?;
    
    // User deposits 10,000 tokens at block 15
                        ...
                        vec![
                            Protostone {
                                ...
                                edicts: vec![
                                    ProtostoneEdict {
                                        id: ProtoruneRuneId { block: 2, tx: 1 },
                                        amount: 120000u128,  // Send all available tokens
                                        output: 1,
                                    }
                                ],
                            }
    ...
    // Wait 10 blocks and withdraw at block 25
    ...
    index_block(&withdrawal_block, 25)?;
    ...
    // Expected rewards: 10 blocks * 1000 rewards/block = 10,000 tokens
    // User was the only one staking, so should get all rewards
    let expected_rewards = 10000u128;
    ...
    // This should detect the 40% variance bug
    assert!(variance_pct <= 5.0, 
        "DETECTED REWARD CALCULATION BUG: got {} rewards, expected {}, variance {:.1}% (>5%)",
        rewards, expected_rewards, variance_pct
    );
