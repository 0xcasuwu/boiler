...
fn test_bond_redemption() -> Result<()> {
    ...
    // Redeem the bond
    let redemption_amount = collection.redeem_bond(&orbital_id, &block_context)?;
...
fn test_mature_bonds_query() -> Result<()> {
    ...
    collection.mint_bond("orbital-1".to_string(), 1000, &block_context)?;
    
    block_context.advance_blocks(25);
    collection.mint_bond("orbital-2".to_string(), 2000, &block_context)?;
    
    block_context.advance_blocks(25);
    collection.mint_bond("orbital-3".to_string(), 3000, &block_context)?;
...
fn test_total_value() -> Result<()> {
    ...
    collection.redeem_bond("orbital-1", &advanced_context)?;
