...
fn create_intricate_temporal_vault_setup(end_reward_block: u128) -> Result<(Block, AlkaneId, u128)> {
    ...
    // Create free_mint with WORKING authorization system
...
// Dynamic MasterChef Pool-Sharing Calculator with Temporal Cap
fn calculate_masterchef_rewards_with_temporal_cap(
    positions: &[Position],
    reward_per_block: u128,
    temporal_cap: u32
) -> std::collections::HashMap<String, u128> {
    println!("🧮 CALCULATING MASTERCHEF POOL-SHARING REWARDS:");
    ...
    for pos in positions {
        events.push((pos.deposit_block, pos.user.clone(), pos.amount, true));  // deposit
        if let Some(withdrawal_block) = pos.withdrawal_block {
            events.push((withdrawal_block, pos.user.clone(), pos.amount, false)); // withdrawal
        }
    }
    ...
    println!("\n📊 DYNAMIC MASTERCHEF POOL PERIODS:");
    for (i, (start_block, end_block, total_staked, active_users)) in pool_periods.iter().enumerate() {
        let effective_end = std::cmp::min(*end_block, temporal_cap);
        let blocks = if *start_block >= temporal_cap {
...
fn test_intricate_temporal_cap_with_overlapping_positions() -> Result<()> {
    ...
    let expected_rewards_map = calculate_masterchef_rewards_with_temporal_cap(
        &positions,
        reward_per_block,
        end_reward_block as u32
    );
    
    // Update expected rewards for each position
    for pos in positions.iter_mut() {
        pos.expected_rewards = expected_rewards_map.get(&pos.user).copied().unwrap_or(0);
    }
