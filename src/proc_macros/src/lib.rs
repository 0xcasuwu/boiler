...
/// Macro for declaring an Alkane contract
#[proc_macro]
pub fn declare_alkane(input: TokenStream) -> TokenStream {
    let input_str = input.to_string();
    
    // Parse the input to extract the type and message type
    let re = regex::Regex::new(r"impl\s+AlkaneResponder\s+for\s+(\w+)\s*\{\s*type\s+Message\s*=\s*(\w+)\s*;\s*\}").unwrap();
    
    if let Some(captures) = re.captures(&input_str) {
        let responder_type = captures.get(1).unwrap().as_str();
        ...
        let expanded = format!(
            r#"
            impl {responder_type} {{
                pub fn dispatch_message(&self, opcode: u128, args: &[u8]) -> Result<alkanes_support::response::CallResponse, anyhow::Error> {{
                    // Convert the opcode to u128
                    let opcode_u128 = opcode as u128;
                    
                    // Parse the arguments into a vector of u128
                    let mut inputs = Vec::new();
                    let mut i = 0;
                    while i + 16 <= args.len() {{
                        let mut buf = [0u8; 16];
                        buf.copy_from_slice(&args[i..i+16]);
                        inputs.push(u128::from_le_bytes(buf));
                        i += 16;
                    }}
                    
                    // Create the message from the opcode and inputs
                    let message = {message_type}::from_opcode(opcode_u128, inputs)?;
        ...
        panic!("Invalid input format for declare_alkane macro");
    }
}
