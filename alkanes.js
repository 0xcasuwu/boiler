...
const encodeProtostone = ({ protocolTag = 1n, edicts = [], pointer = 0, refundPointer = 1, calldata, }) => {
...
const createExecutePsbt = async ({ alkanesUtxos, frontendFee, feeAddress, utxos, account, protostone, provider, feeRate, fee = 0, }) => {
    try {
        if (!alkanesUtxos) {
            throw new errors_1.OylTransactionError(Error('alkanesUtxos is required'));
        }
        ...
        const minTxSize = (0, btc_1.minimumFee)({
            ...
            outputCount: 3 + (feeSatEffective > 0n ? 1 : 0),
        });
